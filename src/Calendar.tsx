...
import { format, startOfMonth, startOfWeek, addDays, isSameMonth, addMonths, isSameDay } from "date-fns";
...
const SchedulCalendarDayCell: React.FC<SchedulCalendarDayCellProps> = ({
  ...
  const cornerClasses = `${isFirstRow && isFirstColumn ? "rounded-tl-lg" : ""} 
                      ...
                      ${isLastRow && isLastColumn ? "rounded-br-lg" : ""}`.trim();

  const cellClasses = `flex flex-col w-32 h-[108px] py-2 border text-sm relative box-border ${
    isCurrentMonth ? "text-black" : "text-gray-400"
  } ${cornerClasses}`;

  const dailyTasks = tasks.filter((task) => task.dateInfo.startDate <= date && task.dateInfo.endDate >= date);

  const renderTasksLabel = (taskSlotIndex: number, filteredTasks: TaskWithSlot[]) => {
    return filteredTasks
      ...
      .map((task) => {
        ...
        const positionClasees = isFirstColumn ? "left-0" : isLastColumn ? "right-0" : "";
        const labelClasses = `absolute flex items-center w-32 h-full text-xs whitespace-nowrap bg-indigo-100 border border-indigo-400 ${borderClasses} ${positionClasees}`;

        return isSameDay(date, task.dateInfo.startDate) ? (
          <div key={`${task.taskInfo.title}-${taskSlotIndex}-${date.toISOString()}`} className={`${labelClasses} z-20`}>
            <div className="absolute w-0.5 h-4 bg-indigo-300 mx-1" />
            <span className="absolute ml-2 border-indigo-400">{task.taskInfo.title}</span>
          </div>
        ) : (
          <div key={`${task.taskInfo.title}-${taskSlotIndex}-${date.toISOString()}`} className={`${labelClasses} z-10`}>
            &nbsp;
          </div>
        );
      });
    ...
    for (const task of tasks) {
      ...
      while (!slotFound) {
        ...
        const isSlotOccupied = tasksWithSlots.some(
          (t) =>
            t.slot === currentSlot &&
            ((t.dateInfo.startDate <= task.dateInfo.startDate && t.dateInfo.endDate >= task.dateInfo.startDate) ||
              (t.dateInfo.startDate <= task.dateInfo.endDate && t.dateInfo.endDate >= task.dateInfo.endDate))
        );
    ...
    <div className="w-full py-8 bg-white rounded-lg">
      <div className="flex flex-col items-center justify-center">
        <header className="flex items-center justify-center w-full gap-8 p-4">
          <button type="button" onClick={prevMonth} className="bg-blue-100 size-8">
            &lt;
          </button>
          <span className="text-[32px] font-semibold">{format(currentDate, "yyyy.MM")}</span>
          <button type="button" onClick={nextMonth} className="bg-blue-100 size-8">
            &gt;
          </button>
        </header>
...
const MiniCalendarDayCell: React.FC<{ children: ReactNode; isCurrentMonth: boolean; isToday: boolean }> = ({
  children,
  isCurrentMonth,
  isToday,
}) => {
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg shadow h-[300px] p-4">
      ...
      <div className="grid grid-cols-7 gap-2 mt-4">
        {daysOfWeek.map((item) => (
          <div className="flex items-center justify-center text-xs size-6 text-zinc-400" key={item}>
            {item}
          </div>
        ))}
        {days.map((day) => (
...
const TodoItem = ({ dateInfo, taskInfo: { title, description } }: Task) => (
  <div className="flex items-center my-4 w-60">
    <div className="flex items-center h-16 pr-4 border-r-4 w-14">{format(new Date(dateInfo.startDate), "MM/dd")}</div>
